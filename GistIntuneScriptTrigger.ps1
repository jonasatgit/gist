...
if ($selection -match '^\d+$' -and $selection -ge 1 -and $selection -le $GistCatalog.Count) {
    $scriptTitle = $GistCatalog.Keys[([int]$selection - 1)]
    $scriptUri = $GistCatalog[([int]$selection - 1)].url
    $author = $GistCatalog[([int]$selection - 1)].author
    $requires_elevated = $GistCatalog[([int]$selection - 1)].requires_elevated
    $ScriptInfo = "`n  Name: {0}`n  Author: {2}`n  Requires elevated rights: {3}`n  URL: {4}" -f $scriptTitle, $scriptUri, $author, $requires_elevated, $scriptUri
